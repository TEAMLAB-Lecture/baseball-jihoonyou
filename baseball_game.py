...
def is_between_100_and_999(user_input_number):
    ...
    result = True if 100 <= num and num <= 999 else False
...
def is_duplicated_number(three_digit):
    ...
    result = True if len(three_digit) != len(set(three_digit)) else False
    # ==================================
    return result
...
def get_strikes_or_ball(user_input_number, random_number):
    ...
    ball = 0
    strikes = 0
    for user_i, user_num in enumerate(user_input_number):
        for ran_i, ran_num in enumerate(random_number):
            if user_num == ran_num:
                if user_i == ran_i:
                    strikes += 1
                else:
